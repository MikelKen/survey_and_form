...
// 2. Silenciar logger de Tigo durante la ejecución de pruebas
vi.mock("@tigo/logger", () => ({
  logger: {
    startTimer: vi.fn(),
    endTimer: vi.fn(),
...
// 3. Helper para simular respuesta HTTP de Express
const mockRes = () => {
  const res = {};
...
describe("Answer Controller - Unit Tests", () => {
  beforeEach(() => {
    vi.clearAllMocks();
  });
  ...
  describe("listSubmissionsByFormController", () => {
    ...
    it("responde con error si el servicio de listado falla", async () => {
      const error = new Error("Error interno");
      error.errorCode = errorCodes.UNKNOWN;
      listSubmissionsByFormService.mockRejectedValue(error);
