...
// Helper para simular respuesta HTTP de Express
const mockRes = () => {
  const res = {};
...
describe("Form Controller - Unit Tests", () => {
  ...
  describe("createFormController", () => {
    ...
    it("responde con status de error si la creación falla en el servicio", async () => {
      ...
      const req = { validated: { title: "Encuesta" }, user: { id: "c101" } };
      const res = mockRes();
  ...
  describe("getFormByIdController", () => {
    it("responde 200 con el formulario encontrado", async () => {
      ...
      getFormByIdService.mockResolvedValue(mockForm);
      ...
      expect(getFormByIdService).toHaveBeenCalledWith(req.validated);
    ...
    it("responde 404 si el formulario no existe", async () => {
      ...
      getFormByIdService.mockRejectedValue(error);
  ...
  describe("listFormsByCreatorController", () => {
    it("responde 200 con la lista paginada de formularios del creador", async () => {
      ...
      expect(listFormsByCreatorService).toHaveBeenCalledWith(
        "c101",
        req.validated,
      );
      expect(res.status).toHaveBeenCalledWith(200);
  ...
  describe("updateFormController", () => {
    ...
    it("responde 409 (Conflict) si intenta editar un formulario ya publicado", async () => {
      ...
      const req = { validated: { id: "f101", title: "Título" } };
      const res = mockRes();
